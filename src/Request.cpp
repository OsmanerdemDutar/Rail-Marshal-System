...
int Request::computeEmergencyScore() const {
    ...
    if(getType() == "SUPPLY"){
        ...
        int x = (getAmount() < 50 ? getAmount() : 50);
        int emergencyLevel = getEmergencyLevel() * 10 + x;
        return emergencyLevel;
    }
    else if(getType() == "RESCUE"){
        int emergencyLevel = getEmergencyLevel() * 10 + getNumPeople() * getRiskMultiplier();
        return emergencyLevel;
    }
    return 0;
...
int Request::computeWorkloadContribution() const {
    ...
    if(getType() == "RESCUE"){
        return getNumPeople() * getRiskMultiplier();
    }
    else if(getType() =="SUPPLY"){
        return getAmount();
    }
